define([
  ...
  YX.fn.initUI = function() {
    this.buildSessions();
    this.showSysMsgCount && this.showSysMsgCount();
  };
  /**
   * 初始化个人信息回调 显示左上角信息
   */
  YX.fn.showMe = function() {
    ...
    this.$userPic.attr('src', Util.getAvatar(user.avatar));
    Util.setCookie('nickName', user.nick);
  ...
  YX.fn.openChatBox = function(account, scene) {
    ...
    this.$messageText && this.$messageText.val('');
    ...
    if (scene == 'p2p') {
      ...
      if (info.account == this.userUID) {
        ...
        this.$chatTitle.find('img').attr('src', Util.getAvatar(info.avatar));
      }
      // 群资料入口隐藏
  ...
  YX.fn.getNick = function(account) {
    // 使用util中的工具方法
    return Util.getNick(account, this.cache);
  };
  ...
  YX.fn.infoProvider = function(data, type) {
    ...
    switch (type) {
      case 'session':
        ...
        if (scene === 'p2p') {
          ...
          if (msg.target === this.userUID) {
            ...
            info.avatar = Util.getAvatar(userInfo.avatar);
          }
        } else {
          //群组
      ...
      case 'friend':
        ...
        info.avatar = Util.getAvatar(data.avatar);
        info.crtSession = this.crtSession;
        break;
      case 'team':
        info.type = data.type;
