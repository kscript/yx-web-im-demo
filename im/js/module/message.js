...
// 自定义myOption时 可以发送本地消息
YX.fn.sendMessage = function(myOption) {
  myOption = myOption || {};
  var scene = myOption.scene || 'p2p';
  var text = myOption.text;
  var to = myOption.to;
  var isLocal = !!myOption.isLocal;
...
YX.fn.getLocalMsgsDone = function(err, data) {
  if (!err) {
    ...
    var array = Util.getAllAccount(data.msgs);
    var that = this;
...
YX.fn.backoutMsg = function(id, data) {
  ...
  var opeNick = Util.getNick(opeAccount);
  if (msg.scene === 'team') {
...
YX.fn.sendTeamNetCallTip = function(option) {
  ...
  option.nick = tmpUser.nickInTeam || Util.getNick(option.account);
  ...
  this.nim.sendTipMsg({
    ...
    scene: 'team',
    to: option.teamId,
    tip: Util.getNick(option.nick) + option.message,
    time: Date.now(),
    isPushable: false,
    isHistoryable: false,
    isRoamingable: false,
...
YX.fn.sendCustomMessage = function(option) {
  ...
  option.nick = tmpUser.nickInTeam || Util.getNick(option.caller);
